  ...
  <div class="div-article">
    ...
    >
      <b-card-text class="bla-card-content">
        ...
        <p class="see-more">
        <b-link :to="{ name: 'article-id', params: { id: article.codeUrl ? article.codeUrl : article.id, article: article} }">Lire la suite</b-link>
        </p>
      </b-card-text>
        ...
        <b-row>
          <b-col cols="6" class="footer-categories">
            ...
            <span v-for="(cat, index) in catLibelles" :key="cat.code">
              <small>
                <b-link v-if="!showCategories" :to="{ name: 'articles', params: { categorie: cat.code} }" class="cat-link">{{ cat.libelle }}<span v-if="index !== catLibelles.length-1">, </span></b-link>
